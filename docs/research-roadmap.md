...
## F. D-041→D-048 closeout and current architecture frontier
...
That sequence motivated a developmental reset rather than another patch to the V0.4 controller scaffold. Accepted [`ADR 0017`](adr/0017-v0.5-direct-differential-drive-centered-dock-boundary.md) defines the V0.5 first-slice boundary:
...
- exactly eight organism-visible channels: two interoceptive, four exteroceptive, and two quantized signed wheel-delta proprioceptive channels;
- continuous wheel-command/response-dependent energy and heat bookkeeping;
- retirement of the V0.4 D-026/D-027/D-030 behavioural/learning scaffold stack from the V0.5 organism.

D-045 then committed the first deterministic V0.5 substrate implementation. It froze the wheel envelope at ±0.6457718232379019 rad per wheel per 0.1 s, wheel radius 0.045 m, track width 0.185 m, exact differential-drive arc integration, deterministic boundary scaling, centred corresponding dock contacts, a 1-degree encoder quantum labelled as an engineering estimate, retained charger/thermal semantics, and continuous actuator bookkeeping. Its evaluator-scripted probe artifact regenerated byte-identically and passed its structural-feasibility check. This is a **substrate-consistency Development result only**: D-045 contains no learner, behavioural controller, calibration curriculum, autonomous docking policy, or confirmatory claim.

D-046 then introduced a fresh 528-weight V0.5 shadow consequence learner after one non-semantic 564-transition raw-wheel curriculum per lifetime. It established useful local action-conditioned predictive structure without giving predictions any causal control role. D-047 audited held-out action-consequence discrimination from that learned state and found strongly channel-dependent competence: wheel consequences were highly discriminable, energy was mixed, beacon discrimination was only moderate, and temperature/contact remained sparse or weak.

D-048 asked whether the unchanged D-046 learner was simply undertrained by repeating the exact same seed-specific curriculum for eight continuous passes, with read-only checkpoints at `0×/1×/2×/4×/8×`. The corrected rerun completed all 20 development seeds. Prequential training error improved across all channels, but held-out beacon discrimination was approximately flat, energy discrimination peaked earlier and then declined, wheel channels remained strong, and contact remained sparse/mixed. This weakens the simple “more of the same experience” explanation without establishing that a larger learner, recurrence, history, or any particular successor is required.

No successor D-stage is authorized by D-048. The current durable architecture frontier is ADR 0018, which establishes the three functional developmental levels and the **Innate Autonomous Viability** milestone. Under that boundary, basic energy-return initiation, beacon-guided reacquisition, organism-visible terminal docking/charging, and fixed baseline return-budget sizing may be constitutive Level-1 mechanisms rather than behaviours that Level 3 must rediscover from scratch. The separate non-authorizing [Return Reserve Margin future question](return-reserve-margin-future-question.md) preserves candidate mathematics without freezing a controller equation.
...
Near-term development now separates a **Viability Track** from a **Learning Track**. The Viability Track works toward the assumption-bounded Innate Autonomous Viability milestone inside ADR 0018. The Learning Track may continue investigating broader experience, representation/capacity, consolidation, and later open-ended learning only through separately authorized work. Neither track authorizes a predicted D-stage number merely by appearing in this roadmap.
