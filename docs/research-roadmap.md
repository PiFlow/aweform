...
## F. D-041→D-045 closeout and current V0.5 frontier
...
The current authorized-but-not-yet-committed frontier is **D-046**, authorized by [issue #165](https://github.com/PiFlow/aweform/issues/165) from D-045 main. D-046 asks whether a fresh 528-weight continuous-action consequence learner can acquire useful one-step and short-rollout prediction after exactly one externally selected, non-semantic 564-transition raw-wheel calibration curriculum per lifetime. The learner remains shadow-only: predictions cannot select, rank, gate, veto, modify, or terminate actions. Its held-out probes and rollouts are evaluator-only and read-only. D-046 has no committed D-record yet, and a positive result would not automatically authorize D-047.
