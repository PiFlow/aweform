...
| `config` | `causal-inherited` | Construction-invariant `EXP003ControllerConfig` (`enter_seek`, `recover`, `exploration_hazard`). Governed by the EXP-003 specification and ADR 0008. Meets the five ADR 0009 B.5 properties, all verified by the checks: initialized before the first `act()`; underlying value immutable for the run; controller-visible binding read-only after initialization (`StationB50Controller.__setattr__` rejects any rebinding, so no caller can swap in a branch-dependent value between `act()` calls); independent of observations/history; and unable to encode a branch bit. A frozen dataclass value alone proves none of these — the binding freeze does. |
...
| `_last_decision` | `diagnostic` | Decision trace (`EXP003ControllerDecision`). Readers: the `last_decision` property and other **external inspection only** — no action-selection branch reads it. No BOHS budget consumed (ADR 0009 E). |
