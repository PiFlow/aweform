"""D-031 learned SEEK scaffold-displacement development probe.

D-031 keeps the merged D-030 learned steering intervention and disables only
...
D031_DEFAULT_DEVELOPMENT_SEEDS: Final[tuple[int, ...]] = tuple(range(18448, 18468))
D031_CANONICAL_VISUALIZATION_SEED: Final[int] = D031_DEFAULT_DEVELOPMENT_SEEDS[0]
D031_HORIZON: Final[int] = 70_000
D031_AUTHORITATIVE_BASE_SHA: Final[str] = (
    "b05d1ee094d999c0acf13126b7374ed11966e3f9"
)
D031_BASE_TREE_SHA: Final[str] = "ceed52790b1fa3726fe2e44a28c6c56f10272258"
D031_SEEK_DELEGATION_PROBABILITY: Final[float] = d026.D026_SEEK_DELEGATION_PROBABILITY
D031_NO_DETRAP_DELEGATION_PROBABILITY: Final[float] = 0.0
D031_EXPLORER_HAZARD: Final[float] = 1.0 / 8.0
D031_LEARNING_RATE: Final[float] = d027.D027_LEARNING_RATE
D031_PLASTIC_STATE_DIMENSION: Final[int] = d027.D027_PLASTIC_STATE_DIMENSION
D031_CHANNELS: Final[tuple[str, ...]] = d027.D027_CHANNELS
D031_OUTPUTS: Final[tuple[str, ...]] = d027.D027_OUTPUTS
D031_FORWARD_OUTPUT_INDEX: Final[int] = D031_OUTPUTS.index("delta_beacon_forward")
D031_STEERING_ACTIONS: Final[tuple[Action, ...]] = (
    Action.TURN_LEFT,
    Action.TURN_RIGHT,
    Action.MOVE_FORWARD,
)
D031_ARM_NAMES: Final[tuple[str, ...]] = (
    "LEARNED_WITH_DETRAP",
    "LEARNED_NO_DETRAP",
    "GREEDY_NO_DETRAP",
)
D031_NO_DETRAP_ARMS: Final[tuple[str, ...]] = (
    "LEARNED_NO_DETRAP",
    "GREEDY_NO_DETRAP",
)
D031_BOUNDARY_CLASSES: Final[tuple[str, ...]] = (
    "FULL_NOMINAL_FORWARD",
    "BOUNDARY_CLIPPED_FORWARD",
    "FULL_STALL_FORWARD",
)
D031_QUARTERS: Final[tuple[str, ...]] = ("Q1", "Q2", "Q3", "Q4")


def _validate_d031_development_seeds(seeds: Sequence[int]) -> tuple[int, ...]:
    """Apply the canonical reservation guard and exact D-031 seed guard."""
    validated = validate_exp003_development_seeds(seeds)
    if validated != D031_DEFAULT_DEVELOPMENT_SEEDS:
        raise ValueError(
            "D-031 requires exactly the frozen development seeds "
            f"{D031_DEFAULT_DEVELOPMENT_SEEDS}; got {validated}"
        )
    return validated


def _validate_d031_seed(seed: int) -> None:
    validated = validate_exp003_development_seeds((seed,))
    if validated[0] not in D031_DEFAULT_DEVELOPMENT_SEEDS:
        raise ValueError(
            "D-031 may execute only predeclared development seeds "
            f"{D031_DEFAULT_DEVELOPMENT_SEEDS}; got {validated}"
        )
...
class D031NoDetrapController(d026.D026Controller):
    """D-026 controller with only false-contact SEEK delegation disabled."""

    seek_delegation_probability = D031_NO_DETRAP_DELEGATION_PROBABILITY

    def __init__(self, policy_rng: np.random.Generator) -> None:
        super().__init__(policy_rng)
        self.false_contact_seek_explorer_calls = 0
        self.explorer = _D031CountingExplorer(policy_rng, self)


class _D031CountingExplorer(StochasticPersistentExplorer):
    """Evaluator counter proving the no-de-trapping SEEK path is unreachable."""

    def __init__(
        self, policy_rng: np.random.Generator, owner: D031NoDetrapController
    ) -> None:
        super().__init__(policy_rng)
        self.owner = owner
...
def _initial_environment(
    ...
    if info != {}:
        raise RuntimeError("D-031 reset crossed the information boundary")
    if environment.body is None or environment.station_center is None:
        raise RuntimeError("D-031 reset did not initialize evaluator geometry")
...
def _query_candidate_predictions(
    ...
    predictions = {
        action: learner.predict(current, action) for action in D031_STEERING_ACTIONS
    }
...
def _exact_argmax(scores: dict[Action, float]) -> tuple[Action, ...]:
    ...
    return tuple(
        action for action in D031_STEERING_ACTIONS if scores[action] == maximum
    )
...
def _evaluate_steering_branches(
    ...
    return {
        ...
        for action in D031_STEERING_ACTIONS
    }
...
def _run_arm(
    ...
    *,
    ...
    if arm not in D031_ARM_NAMES:
        raise ValueError(f"unknown D-031 arm: {arm}")
    if seed_validator is None:
        _validate_d031_seed(seed)
    else:
        seed_validator(seed)
    ...
    if arm in D031_NO_DETRAP_ARMS:
        controller = D031NoDetrapController(streams.policy)
    else:
    ...
    while not (terminated or truncated):
        if environment.body is None or environment.station_center is None:
            raise RuntimeError("D-031 evaluator geometry disappeared")
        ...
        if arbitration is not None:
            if current.charging_contact or mode_after is not d026.D026Mode.SEEK:
                raise RuntimeError("D-031 arbitration used outside false-contact SEEK")
            ...
            if not arbitration.delegated and evaluator_diagnostics:
                if not predictions or learned_action is None:
                    raise RuntimeError("D-031 evaluator prediction query missing")
        ...
        if reward != 0.0 or info != {}:
            raise RuntimeError("D-031 real transition crossed the boundary")
        telemetry = environment.last_transition
        if telemetry is None:
            raise RuntimeError("D-031 real transition produced no telemetry")
        ...
        if (
            ...
            raise RuntimeError("D-031 executed pre-update prediction changed")
        ...
        if (
            ...
            actual_forward = {
                candidate.name: branches[candidate].delta[D031_FORWARD_OUTPUT_INDEX]
                for candidate in D031_STEERING_ACTIONS
            }
            truth = _exact_argmax(
                {
                    candidate: branches[candidate].delta[D031_FORWARD_OUTPUT_INDEX]
                    for candidate in D031_STEERING_ACTIONS
                }
            )
            learned_scores = {
                candidate: current.beacon.forward
                + predictions[candidate].values[D031_FORWARD_OUTPUT_INDEX]
                for candidate in D031_STEERING_ACTIONS
            }
            decision_records.append(
                {
                    "transition": transition_index,
                    "quarter": D031_QUARTERS[
    ...
    if environment.last_transition is None:
        raise RuntimeError("D-031 run ended without final telemetry")
    outcome = (
        "FULL_CYCLE"
    ...
    no_detrap_explorer_calls = (
        ...
        if isinstance(controller, D031NoDetrapController)
        else 0
    )
    delegation_summary = {
        ...
        "delegation_probability": (
            D031_SEEK_DELEGATION_PROBABILITY
            if arm == "LEARNED_WITH_DETRAP"
            else D031_NO_DETRAP_DELEGATION_PROBABILITY
        ),
        "delegation_probability_expression": (
            "1.0 / 3.0" if arm == "LEARNED_WITH_DETRAP" else "0.0"
        ),
        "explorer_internal_hazard": D031_EXPLORER_HAZARD,
    ...
    return {
        ...
        "isolation": {
            ...
            "no_false_contact_seek_explorer_call": (
                arm not in D031_NO_DETRAP_ARMS or no_detrap_explorer_calls == 0
            ),
        },
...
def _aggregate_diagnostics(records: Sequence[dict[str, object]]) -> dict[str, object]:
    actual_sums = {action.name: 0.0 for action in D031_STEERING_ACTIONS}
    margins: list[float] = []
...
def _run_d031_seed(seed: int, *, horizon: int = D031_HORIZON) -> dict[str, object]:
    ...
    for arm in D031_NO_DETRAP_ARMS:
        ...
        if not comparison["real_summary_exact_equal"]:
            raise RuntimeError(f"D-031 {arm} evaluator branch isolation failed")
    ...
    if all(successful(arm_results[arm]) for arm in D031_ARM_NAMES):
...
def run_d031_probe(
    seeds: Sequence[int] = D031_DEFAULT_DEVELOPMENT_SEEDS,
    *,
    horizon: int = D031_HORIZON,
    executed_commit_sha: str | None = None,
) -> dict[str, object]:
    """Run exactly the frozen three-arm D-031 development protocol."""
    development_seeds = _validate_d031_development_seeds(seeds)
    if horizon != D031_HORIZON:
        raise ValueError("D-031 requires the frozen 70,000-transition horizon")
    executed_sha = _validate_executed_commit_sha(executed_commit_sha)
    results = [_run_d031_seed(seed, horizon=horizon) for seed in development_seeds]
    ...
    return {
        ...
        "authoritative_base_sha": D031_AUTHORITATIVE_BASE_SHA,
        "base_tree_sha": D031_BASE_TREE_SHA,
        "implementation_probe_sha": executed_sha,
        "development_seeds": list(development_seeds),
        "horizon": D031_HORIZON,
        "timestep_seconds": D020PhysicalConfig().dt_seconds,
        "simulated_duration_seconds": D031_HORIZON * D020PhysicalConfig().dt_seconds,
        ...
        "seed_policy": {
            ...
            "exact_declared_seeds": list(D031_DEFAULT_DEVELOPMENT_SEEDS),
            "formal_reservation_guard_preserved": True,
            "exact_d031_guard": True,
        },
        "freeze": {
            "controller": (
            ...
            "horizon": D031_HORIZON,
            "channels": list(D031_CHANNELS),
            "outputs": list(D031_OUTPUTS),
            "feature_vector": ["bias=1.0", *D031_CHANNELS],
            "learning_rate": D031_LEARNING_RATE,
            "plastic_state_dimension": D031_PLASTIC_STATE_DIMENSION,
            "candidate_actions": [action.name for action in D031_STEERING_ACTIONS],
            "wait_excluded": True,
            "forward_output": "delta_beacon_forward",
            "forward_output_index": D031_FORWARD_OUTPUT_INDEX,
            ...
            "delegation_probability_arm_a": D031_SEEK_DELEGATION_PROBABILITY,
            "delegation_probability_arm_b_c": D031_NO_DETRAP_DELEGATION_PROBABILITY,
        ...
        "learned": {
            ...
            "weight_count": D031_PLASTIC_STATE_DIMENSION,
...
def run_d031_lifetime_trace(
    seed: int = D031_CANONICAL_VISUALIZATION_SEED,
    *,
    arm: str = "LEARNED_WITH_DETRAP",
    horizon: int = D031_HORIZON,
) -> tuple[d025.D025TransitionTrace, ...]:
    """Run one branch-disabled D-031 arm and retain its causal trace."""
    _validate_d031_seed(seed)
    if arm not in D031_ARM_NAMES:
        raise ValueError(f"unknown D-031 arm: {arm}")
    if horizon != D031_HORIZON:
        raise ValueError(
            "D-031 lifetime traces require the frozen 70,000-transition horizon"
        )
    ...
    if not trace or result["transitions"] != len(trace):
        raise RuntimeError("D-031 lifetime trace disagrees with the run")
    ...
    if not naturally_terminated and not horizon_censored:
        raise RuntimeError("D-031 lifetime trace has an invalid terminal state")
    return tuple(trace)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the D-031 scaffold probe.")
    parser.add_argument(
        "--seeds", type=int, nargs="+", default=list(D031_DEFAULT_DEVELOPMENT_SEEDS)
    )
    parser.add_argument("--horizon", type=int, default=D031_HORIZON)
...
def main() -> None:
    ...
    if args.output is None:
        ...
        print(f"D-031 result written to {args.output}")
