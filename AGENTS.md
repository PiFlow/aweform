...
Committed D-series development currently extends through **D-045**. D-041 completed the evaluator-only sensory-sufficiency audit; D-042→D-044 closed the V0.4 embodiment/docking line; and D-045 committed the deterministic V0.5 substrate with signed bilateral wheel-delta control, differential-drive kinematics, centred under-body dual contact, exactly eight organism-visible channels, and continuous actuator bookkeeping. D-045 contains no learner, behavioural controller, calibration curriculum, or autonomous docking policy. The exact committed ledger is [`development/INDEX.md`](development/INDEX.md).

The current authorized-but-not-yet-committed Development stage is **D-046**, authorized by [issue #165](https://github.com/PiFlow/aweform/issues/165) from main `4b32e49997cba6b223057b7e1946a97ba6fe9235`. It permits one at-most-once non-semantic raw-wheel calibration curriculum and a fresh shadow-only V0.5 consequence learner; predictions may not influence action selection. D-046 does not become a committed D-record until its record is merged. For any later or changed work, read the exact current GitHub authorization issue/PR rather than inferring permission from ADR 0017's sequence.
