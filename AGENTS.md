...
- **one-go** (default): one PR at a time. Every PR still requires a Codex-Sol `APPROVE`/`REQUEST-CHANGES` comment before merge (see Roles above). Once that's in, Claude-Sonnet5 asks Flow to authorise each merge individually before merging it.
