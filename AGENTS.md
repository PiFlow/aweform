...
Committed D-series development currently extends through D-044, including the completed evaluator-only D-041 sensory-sufficiency audit and the D-042→D-044 V0.4 embodiment/docking characterizations. The exact committed ledger is [`development/INDEX.md`](development/INDEX.md). ADR 0017 is accepted as the prospective V0.5 durable boundary, but implementation permission for D-045 or any later stage must come from the exact current GitHub authorization issue/PR. Do not infer implementation authorization from ADR 0017's prospective sequence.
